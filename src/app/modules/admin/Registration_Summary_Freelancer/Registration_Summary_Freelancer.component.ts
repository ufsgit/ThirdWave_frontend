...
@Component({
selector: 'app-Registration_Summary_Freelancer',
templateUrl: './Registration_Summary_Freelancer.component.html',
styleUrls: ['./Registration_Summary_Freelancer.component.css'],
providers: [
...
export class Registration_Summary_FreelancerComponent implements OnInit {
    ...
    Branchwise_data = [  ];
    Data_Bar = [  ];
    Branchwise_columnNames = ['Source', 'Count'];
    ...
    Look_In_Date_From_Freelancer_Dashboard:number=2;
    Search_FromDate_From_Freelancer_Dashboard: Date = new Date();
    Search_ToDate_From_Freelancer_Dashboard: Date  = new Date();
...
ngOnInit() 
{
  debugger
    ...
    this.Look_In_Date_From_Freelancer_Dashboard = Number(localStorage.getItem("Look_In_Date_From_Freelancer_Dashboard"));
    localStorage.setItem("Look_In_Date_From_Freelancer_Dashboard","2");

    if(this.Look_In_Date_From_Freelancer_Dashboard === 1)
    {
        this.Search_FromDate_From_Freelancer_Dashboard = new Date(localStorage.getItem("Search_FromDate_From_Freelancer_Dashboard"))
        this.Search_ToDate_From_Freelancer_Dashboard = new Date(localStorage.getItem("Search_ToDate_From_Freelancer_Dashboard"))
        this.Look_In_Date = true;
    }
    else if(this.Look_In_Date_From_Freelancer_Dashboard === 0)
    {
        this.Search_FromDate_From_Freelancer_Dashboard = new Date(localStorage.getItem("Search_FromDate_From_Freelancer_Dashboard"))
        this.Search_ToDate_From_Freelancer_Dashboard = new Date(localStorage.getItem("Search_ToDate_From_Freelancer_Dashboard"))
...
Page_Load()
{
    ...
    debugger;
    if(this.Look_In_Date_From_Freelancer_Dashboard === 0 || this.Look_In_Date_From_Freelancer_Dashboard === 1)
    {
        this.Search_FromDate = this.Search_FromDate_From_Freelancer_Dashboard;
        this.Search_ToDate = this.Search_ToDate_From_Freelancer_Dashboard;
    }
    else
    {
    ...
    this.Get_Menu_Status(155,this.Login_User);
...
Search_Lead_button1(Freelancer_Manager_User_Id) 
{
    this.Black_Start =1;
    this.Black_Stop = this.Page_Length_;
    this.Red_Start = 1;
    this.Total_Rows=0;
    this.Red_Stop = this.Page_Length_;
    this.Search_Registration_Report(Freelancer_Manager_User_Id);
}
Search_More_Options()
{
...
Export_Summary()
{
        this.Student_Service_.exportExcel(this.Student_Data_Search,'Registration_Report_Summary')
...
View_Details(User_Details_Id,ViewBranch)
...
this.View_Branch   =ViewBranch;

this.Freelancer_Manager_User_Id = User_Details_Id
this.Search_Registration_Report(this.Freelancer_Manager_User_Id)
}
...
var value = 1, dept_id=0,User_Id=0,search_name_='0',look_In_Date_Value=0,branch_id=0;
    ...
    debugger
    this.Student_Service_.Student_Registration_Summary_Freelancer(moment(this.Search_FromDate).format('YYYY-MM-DD'), moment(this.Search_ToDate).format('YYYY-MM-DD'), branch_id, look_In_Date_Value,this.Login_User)
.subscribe(Rows => 
{debugger
    ...
    for(var j=0;j<Branchwise_data_temp.length;j++){
        this.Total_Registrations=Number(this.Total_Registrations)+Number(Branchwise_data_temp[j].No_of_Registration)
    }
    this.issLoading = false;
    var result = [];
    ...
    {
        result.push([Branchwise_data_temp[i].User, Branchwise_data_temp[i].No_of_Registration]);
       //this.Enquiry_Source_columnNames.push(Enquiry_Source_data_temp[i].Enquiry_Source_Name);
    }
        
    
    //var data_temp = new google.visualization.DataTable(result)
    this.Branchwise_columnNames.push('User_Details_Name')
    this.Branchwise_columnNames.push('Registration_Count')
...
Search_Registration_Report(User_Id)
{
    this.Summary=false
    this.Report=true
   // log(i)
...
var value = 1, dept_id=0,search_name_='0',look_In_Date_Value=0,branch_id=0;
    ...
    this.Student_Service_.Search_Registration_Report_Freelancer(moment(this.Search_FromDate).format('YYYY-MM-DD'),
     moment(this.Search_ToDate).format('YYYY-MM-DD'), value, search_name_, dept_id,branch_id, User_Id,  
     look_In_Date_Value, this.Black_Start, this.Black_Stop, this.Login_User, this.Red_Start, this.Red_Stop,this.View_Branch,this.Freelancer_Manager_User_Id)
.subscribe(Rows => 
{
    ...
    this.Total_Data=0;
    this.Student_Data_Search = Rows.returnvalue.Leads;
    var data1=Rows.returnvalue.Leads;
    this.Total_Data=data1.length;
