...
@Component({
	selector: "app-Refund_Confirmation",
	templateUrl: "./Refund_Confirmation.component.html",
	styleUrls: ["./Refund_Confirmation.component.css"],
	providers: [
		{
			provide: DateAdapter,
...
export class Refund_ConfirmationComponent implements OnInit {
	...
	Page_Load() {
		...
		this.Get_Menu_Status(92, this.Login_User);
		this.Get_Menu_Status(92, this.Login_User);
		...
		this.Search_Refund_Confirmation();
	...
	Search_Refund_Confirmation() {
		...
		this.Student_Service_.Search_Refund_Confirmation(
	...
	Refund_Approve(Fees_Receipt_Id, Student_Id_temp_, index) {
		...
		dialogRef.afterClosed().subscribe((result) => {
			if (result == "Yes") {
				...
				this.Student_Service_.Refund_Approve(
					Fees_Receipt_Id,
					Student_Id_temp_,
					this.Login_User
				).subscribe(
					(Save_status) => {
						if (Number(Save_status[0][0].Fees_Receipt_Id_) > 0) {
							...
							const dialogRef = this.dialogBox.open(DialogBox_Component, {
								panelClass: "Dialogbox-Class",
								data: { Message: "Accounts Confirmed", Type: "false" },
							});
	...
	Remove_Refund_Approval(Fees_Receipt_Id, index) {
		...
		dialogRef.afterClosed().subscribe((result) => {
			if (result == "Yes") {
				this.issLoading = true;
				this.Student_Service_.Remove_Refund_Approval(Fees_Receipt_Id,
					this.Login_User).subscribe(
					(update_status) => {
						if (update_status[0][0].Fees_Receipt_Id_ > 0) {
							const dialogRef = this.dialogBox.open(DialogBox_Component, {
								panelClass: "Dialogbox-Class",
								data: { Message: "Accounts Rejected", Type: "false" },
							});
							// this.Total_Rows = this.Total_Rows - this.Student_Data.length;
							this.Search_Refund_Confirmation();
							if (Number(this.Login_User) != update_status[0][0].To_User_Id_) {
								var message = {
									...
									To_User: update_status[0][0].To_User_Id_,
