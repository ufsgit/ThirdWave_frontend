...
isTypeaheadOpen = false;
  ...
  saveChangeUserPermanent:Boolean = false;
	  private _: any;
...
CC_Users_List_: User_Details = new User_Details();
  ...
  expandedCreatedUser: { [key: number]: boolean } = {};
expandedDepartmentStatus: { [key: number]: boolean } = {};
expandedChangeStatusUser: { [key: number]: boolean } = {};
expandedChangeStatusAdmin: { [key: number]: boolean } = {};
	  //Student_Course_Apply_:Student_Course_Apply= new Student_Course_Apply;
...
ApplicationStatus1: string = '';
	  ...
	  Followup_Users_Data_Full: any[] = []; // Add this to your component
	...
	Name_Show:string;
	showMobileColumn: boolean=true;

	
	/** Added on 20-11-2024 */

	greaterThanZeroIntakeData = [];
...
Followup_Users_Data_Filter: any[] = [];
CC_Users_Data: any[] = [];
CC_Users_Data_Filter: any[] = [];
...
Search_Department_User_Typeahead(event: any) {
	...
	if (
		...
		this.Followup_Users_Data == null ||
		this.Followup_Users_Data.length == 0
	) {
		this.issLoading = true;
		
		this.Student_Service_.Search_Department_User_Typeahead_changestaff(
			...
			(Rows) => {
				if (Rows != null) {
					this.Followup_Users_Data = Rows[0]; // Store full data
					this.Followup_Users_Data_Filter = []; // Initialize filter array
					this.issLoading = false;
					
					// Apply initial filter
					for (var i = 0; i < this.Followup_Users_Data.length; i++) {
						if (
							this.Followup_Users_Data[i].User_Details_Name.toLowerCase().includes(Value)
						) {
...
Search_Department_User_Typeahead_CC(event: any) {
	var Value = "";
	if (event.target.value == "") Value = "";
	else Value = event.target.value.toLowerCase();
	...
	// Fetch data if not already loaded
	if (
		this.CC_Users_Data == undefined ||
		this.CC_Users_Data == null ||
		this.CC_Users_Data.length == 0
	) {
		this.issLoading = true;
		...
		this.Student_Service_.Search_Department_User_Typeahead_changestaff(
			...
			(Rows) => {
				if (Rows != null) {
					this.CC_Users_Data = Rows[0]; // Store full data
					this.CC_Users_Data_Filter = []; // Initialize filter array
					this.issLoading = false;
					
					// Apply initial filter
					for (var i = 0; i < this.CC_Users_Data.length; i++) {
						if (
							this.CC_Users_Data[i].User_Details_Name.toLowerCase().includes(Value)
						) {
							this.CC_Users_Data_Filter.push(this.CC_Users_Data[i]);
						}
					}
				}
			},
...
debugger
		...
		this.myInnerHeightlist = this.myInnerHeight - 150;
		
		this.myInnerHeight = this.myInnerHeight - 170;
		this.myInnerHeighttemp1=this.myInnerHeightlist;

		 this.myInnerHeightOriginal = this.myInnerHeight;
	...
	// Get_Intake_Count() {
	// 	this.Student_Service_.Get_Intake_Count().subscribe(
	// 		(Rows) => {
	// 			this.greaterThanZeroIntakeData = Rows[0]
...
debugger
				...
				// this.countryIntakeData.forEach((year) => {
	...
	Search_Lead_function(View_Type_t) {	
		...
		this.Pointer_Stop_ = this.Page_Length_;
		this.Total_List_Rows = 0;
		console.log('this.Search_Types33: ', this.Search_Types);
...
this.Search_Application_List()
	...
	Search_Application_List() {
		...
		var Is_View_ ,Enrolled_Application_View_,Department_Status_Id_ = 0,search_name_='',search_application_no_=0,look_In_Date_Value=0,Course_Id=0,Country_Id=0,University_Id=0,Intake_Id=0,Intake_Year_Id=0,Department_Status_Id=0,Created_User_Id=0,To_User_Id=0;
...
debugger
    ...
    if (this.Navbar_Leads_View_Menus == 4) {
        ...
        if (this.Navbar_Leads_View_Menus == 3) {
            ...
            // Default to menu 3 if no specific permissions
            this.Navbar_Leads_View_Menus = 3;
            Entry_type_ = 3;
        }
    }
}
		...
		// if (this.Navbar_Leads_View_Menus == 4)
		// {Entry_type_ =2}
			console.log('his.Application_Country_ : ', this.Application_Country_ );
	...
	console.log('this.Course_: ', this.Course_);
...
console.log('Entry_type_: ', Entry_type_);
console.log('this.View_Type_11: ', this.View_Type_);
console.log('application_status_: ', application_status_);
debugger
		this.Student_Service_.Search_Application_List(moment(this.Search_FromDate).format('YYYY-MM-DD'), moment(this.Search_ToDate).format('YYYY-MM-DD'),
			...
			// ,University_Id,
			Intake_Id,Intake_Year_Id,Department_Status_Id,To_User_Id,
			Enrolled_Application_View_,this.Pointer_Start_,
			this.Pointer_Stop_,
			this.Page_Length_,Is_View_,Department_Status_Id_,search_name_,
			look_In_Date_Value,followup_user_selection,this.View_Type_,Entry_type_,search_application_no_,application_status_,Created_User_Id
			...
			(Rows) => {
				...
				const categories = this.ApplicationDetails_Data.map(detail => detail['user_category']);
			
				// console.log('categories: ', categories);
	
				this.showMobileColumn = categories.includes(2);
				// console.log('this.showMobileColumn : ', this.showMobileColumn );
...
// console.log("this.ApplicationDetails_Data.length2x",this.ApplicationDetails_Data.length);
// console.log("this.Total_List_Rowsx",this.Total_List_Rows);
...
Load_Conditions_Subdata_Edit(Application_details_Id_)
{
	...
	Clr_Bph_Status() {
		  this.saveChangeUserPermanent = false;
		if (
...
debugger
    ...
    if (
		...
		if (this.Intake_Date_Year_Check) { 
			console.log('this.Intake_Date_Year_Checkkkk: ', this.Intake_Date_Year_Check);
			// if (
...
this.showIntakeYearDiv = false;
    ...
    debugger
		...
		if (
			this.CC_Users_List_ == null ||
			this.CC_Users_List_ == undefined ||
			this.CC_Users_List_.User_Details_Id == undefined ||
			this.CC_Users_List_.User_Details_Id == null
		) {
			const dialogRef = this.dialogBox.open(DialogBox_Component, {
				panelClass: "Dialogbox-Class",
				data: { Message: "Select From User", Type: "3" },
			});
			return;
		}
...
this.Status_Change_Data_.From_User_Id =this.CC_Users_List_.User_Details_Id;
        this.Status_Change_Data_.From_User_Name =this.CC_Users_List_.User_Details_Name;
        ...
        this.Status_Change_Data_.Offerletter_Type_Name =
            ...
            this.Status_Change_Data_.Student_Id=this.Student_Id;
               ...
               this.Status_Change_Data_.SaveChangeUserPermanent = this.saveChangeUserPermanent;
...
debugger
                this.Student_Service_.Save_Application_Change_User({
                    ...
                    details: this.selectedApplicationsIdDetails,
                    duration:this.ApplicationDetails_.Department_Status_Duration,
					 SaveChangeUserPermanent: this.saveChangeUserPermanent
                  }).subscribe(
                    (Save_status) => {
	...
	Create_New_Company()
{
	...
	Edit_Student_Notification(Student_Id, i) {
		...
		localStorage.setItem("From_Application", "1");
	...
	Search_Country_Typeahead(event: any) {
		...
		if (this.Country_Data == undefined || this.Country_Data.length == 0) {
			console.log('this.Country_Data: ', this.Country_Data);
			this.issLoading = true;

			this.Country_Service_.Search_Country_Typeahead(Value).subscribe(
				(Rows) => {
					console.log('Rows countries: ', Rows);
					debugger
					if (Rows != null) {
	...
	Search_University_Typeahead_Country(event: any) {
		console.log('this.Application_Country_.Country_Id: ', this.Application_Country_.Country_Id);
	...
	console.log('his.Application_Country_: ', this.Application_Country_);
		...
		if (this.University_Data == undefined || this.University_Data.length == 0) {
			...
			this.University_Service_.Search_University_Typeahead_Country(Value,this.Application_Country_.Country_Id).subscribe(
				(Rows) => {
					console.log('Rows university: ', Rows);
					debugger
					if (Rows != null) {
						this.University_Data = Rows[0];
						this.University_Data_Filter_2 = [];
...
// Modified Country_Change1 method - remains the same
Country_Change1() {
    this.University_1 = null;
...
// Modified University_Change1 method - DON'T clear country related data
University_Change1() {
	console.log('University changed, updating courses...',this.University_1);
	console.log('application countryyysyss',this.Application_Country_);
	
    // Only clear course and department related data, NOT country data
    this.Course_ = null;
    this.Course_Data = [];
    this.Course_Data_Filter = [];
    this.Department_Status_Data = [];
    this.Department_Status_Data_Filter_2 = [];
    this.selectedCourses = []; // Clear selected courses when university changes
...
debugger
	...
	// removeZeroCountIntakes(): void {

	// 	debugger;


	//  // Filter the data
	//  const filteredData = this.combinedIntakeData.filter(combinedItem => {
	// 	debugger;
	// 	// Check if the combo exists in greaterThanZeroIntakeData
	// 	const found = this.greaterThanZeroIntakeData.some(
	// 	  greaterThanZeroItem =>
	// 		greaterThanZeroItem.Intake_Id === combinedItem.Intake_Id &&
	// 		greaterThanZeroItem.Intake_Year_Id === combinedItem.Intake_Year_Id
	// 	);
	
	// 	// Keep the item if it exists in greaterThanZeroIntakeData
	// 	return found;
	//   });
	
	//   // Add the new default item after filtering
	//   this.combinedIntakeData1 = [
	// 	...filteredData,
	// 	{
	// 	  Intake_Year_Id: 0,
	// 	  Intake_Id: 0,
	...
	//   console.log('Filtered combinedIntakeData1:', this.combinedIntakeData1);
	//   }
   ...
   Get_ApplicationDetailswise_History(application_details_id_, feesdetails_id_) {
	...
	this.issLoading = true;
	debugger
	console.log('feesdetails_id_: ', feesdetails_id_);
	console.log('application_details_id_: ', application_details_id_);
	console.log('this.Application_Id_Temp_: ', this.Application_Id_Temp_);
...
Document_change(Process_id,Department_Status_Id){
	...
	debugger		
			if (Rows != null) {
				...
				console.log('this.ApplicationDetails_.Task_Status: ', this.ApplicationDetails_.Task_Status);
				this.ApplicationDetails_.Department_Status_Duration=Rows[7][0].Status_Order
				console.log('this.ApplicationDetails_.Department_Status_Duration: ', this.ApplicationDetails_.Department_Status_Duration);
				this.Transfer_temp_name=Rows[7][0].Transfer_Department_Name
				console.log('this.Transfer_temp_name: ', this.Transfer_temp_name);
				
				this.process_Task_Data_pending = Rows[8];
				console.log('this.process_Task_Data_pending: ', this.process_Task_Data_pending);
				this.ApplicationDetails_.Fees_status_profile = Rows[9][0].Fees_status_profile;
				console.log('this.ApplicationDetails_.Fees_status_profile : ', this.ApplicationDetails_.Fees_status_profile );
				this.ApplicationDetails_.Is_Registered = Rows[10][0].Is_Registered_profile;
				console.log('this.ApplicationDetails_.Is_Registered: ', this.ApplicationDetails_.Is_Registered);
this.ApplicationDetails_.Deadline_Date = new Date();

this.user_category=Rows[11][0].user_category_student

console.log('this.fees_check: ', this.fees_check);
console.log('this.Intake_Date_Year_Check: ', this.Intake_Date_Year_Check);
...
console.log('this.user_category: ', this.user_category);
...
More_Options_function() {
    ...
    if (this.More_Options_View == true) {
        // Closing More Options - restore original heights
        this.myInnerHeightlist = this.myInnerHeighttemp1;
        this.myInnerHeight = this.myInnerHeightOriginal; // Restore original table height
        this.More_Options_View = false;
    } else {
        // Opening More Options - reduce heights to accommodate extra fields
        this.More_Options_View = true;
        this.myInnerHeightlist = this.myInnerHeighttemp1 - 60;
        this.myInnerHeight = this.myInnerHeightOriginal - 60; // Reduce table height by same amount
    }
...
this.Department_Status_Data_Filter_2=[];
  ...
  cat_check(user_category) {
    // Map the ApplicationDetails_Data to extract user_category values and then check with includes
    const categories = this.ApplicationDetails_Data.map(detail => detail['user_category']);
	
    return  categories.includes(2);
}
...
Search_Application_StatusforChangeStatus_Typeahead(event: any, Source: number) {
    ...
    // Check if we need to load all statuses or filtered ones
    if (this.loadAllStatuses) {
        this.loadAllStatusesData(Value);
    } else {
        this.loadFilteredStatusesData(Value);
    }
}
// Function to handle checkbox toggle
toggleLoadAllStatuses() {
    // Clear existing data when toggling
    this.Department_Status_Mode_Data1 = [];
    this.Department_Status_Mode_Data1_Filter = [];
    this.Department_Status_Mode1_ = null;
    
    // Reload data based on current checkbox state
    if (this.loadAllStatuses) {
        this.loadAllStatusesData("");
    } else {
        // Reset to normal filtered mode
        this.Search_Application_StatusforChangeStatus_Typeahead({target: {value: ""}}, 1);
    }
}
// Function to load all statuses
loadAllStatusesData(searchValue: string = "") {
    this.issLoading = true;
    
    // Call service to get all statuses
    this.Country_Service_.Search_All_Application_Statuses(
        this.Login_User, 
        this.selectedApplicationsIdDetails, 
        this.Student_Id
    ).subscribe(
        (Rows) => {
            if (Rows != null) {
                this.Department_Status_Mode_Data1 = Rows[0];
                this.filterStatusData(searchValue);
            }
            this.issLoading = false;
        },
        (error) => {
...
// Function to load filtered statuses (original behavior)
loadFilteredStatusesData(searchValue: string = "") {
    if (this.Department_Status_Mode_Data1 == undefined || this.Department_Status_Mode_Data1.length == 0) {
        this.issLoading = true;
        
        this.Country_Service_.Search_Application_StatusforChangeStatus_Typeahead(
            searchValue, 
            this.Login_User, 
            this.selectedApplicationsIdDetails, 
            this.Student_Id
            ...
            (error) => {
                this.issLoading = false;
                console.error('Error loading filtered statuses:', error);
            }
        );
    } else {
        this.filterStatusData(searchValue);
...
// Helper function to filter status data
filterStatusData(searchValue: string) {
    this.Department_Status_Mode_Data1_Filter = [];
    for (var i = 0; i < this.Department_Status_Mode_Data1.length; i++) {
        if (this.Department_Status_Mode_Data1[i].Department_Status_Name.toLowerCase().includes(searchValue.toLowerCase())) {
            this.Department_Status_Mode_Data1_Filter.push(this.Department_Status_Mode_Data1[i]);
        }
    }
}
showFullCreatedUser(index: number): void {
  this.expandedCreatedUser = { ...this.expandedCreatedUser, [index]: true };
}

hideFullCreatedUser(index: number): void {
  this.expandedCreatedUser = { ...this.expandedCreatedUser, [index]: false };
}

showFullDepartmentStatus(index: number): void {
  this.expandedDepartmentStatus = { ...this.expandedDepartmentStatus, [index]: true };
}

hideFullDepartmentStatus(index: number): void {
  this.expandedDepartmentStatus = { ...this.expandedDepartmentStatus, [index]: false };
}

showFullChangeStatusUser(index: number): void {
  this.expandedChangeStatusUser = { ...this.expandedChangeStatusUser, [index]: true };
}

hideFullChangeStatusUser(index: number): void {
  this.expandedChangeStatusUser = { ...this.expandedChangeStatusUser, [index]: false };
}

showFullChangeStatusAdmin(index: number): void {
  this.expandedChangeStatusAdmin = { ...this.expandedChangeStatusAdmin, [index]: true };
}

hideFullChangeStatusAdmin(index: number): void {
  this.expandedChangeStatusAdmin = { ...this.expandedChangeStatusAdmin, [index]: false };
}
