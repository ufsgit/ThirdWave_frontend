...
import { Leave_Status } from "app/models/Leave_Status";
import { Leave_Management } from "app/models/Leave_Management";
...
@Component({
	selector: "app-Leave_Management",
	templateUrl: "./Leave_Management.component.html",
	styleUrls: ["./Leave_Management.component.css"],
	providers: [
		{
			provide: DateAdapter,
...
export class Leave_ManagementComponent implements OnInit {
	...
	Leave_Management_: Leave_Management = new Leave_Management();
	Temp_Date_Followup: Date;
	Student_Task_Data: Student_Task[];

	Task_Status_Data: Task_Status[];
	Leave_Status_Data: Leave_Status[];
	...
	isTransferVisible: boolean = false; 
	...
	Leave_Status_:Leave_Status = new Leave_Status();
	Leave_Status_search_: Leave_Status = new Leave_Status();
	Leave_Status_Temp_search: Leave_Status = new Leave_Status();
	Leave_Status_Temp: Leave_Status = new Leave_Status();
	Leave_Status_Data_search: Leave_Status[];
	Leave_Status_Id_: number = 0;
...
if(this.Assign_User_Dashboard_==0){this.Assign_User_=0}
	...
	Page_Load() {
		...
		this.Get_Menu_Status(171, this.Login_User);
		...
		this.myInnerHeightTwo = this.myInnerHeight - -20;
	}
...
debugger
	...
	Get_Student_PageLoadData_Dropdowns() {
		...
		this.Student_Service_.Get_Student_PageLoadData_Dropdowns().subscribe(
			(Rows) => {
				...
				this.Leave_Status_Data = Rows[24].slice();
				this.Leave_Status_Temp.Leave_Status_Id = 0;
				this.Leave_Status_Temp.Status_Name = "Select";
				this.Leave_Status_Data.unshift(Object.assign({}, this.Leave_Status_Temp));
				this.Leave_Status_ = this.Leave_Status_Data[0];
	...
	Add_CAS_Followup(Student_Task_Id, Student_Id, task_item_id, task_group_id) {
		...
		// );
		this.Leave_Management_.FromDate = new Date();
		this.Leave_Management_.FromDate = this.New_Date(
			this.Leave_Management_.FromDate
		);
	...
	Save_Leave_Management() {
		...
		if (
			this.Leave_Status_ == undefined ||
			this.Leave_Status_ == null ||
			this.Leave_Status_.Leave_Status_Id == undefined ||
			this.Leave_Status_.Leave_Status_Id == 0
		) {
			const dialogRef = this.dialogBox.open(DialogBox_Component, {
				panelClass: "Dialogbox-Class",
				data: { Message: "Select Status", Type: "3" },
		...
		//     }
		if (
			this.Leave_Management_.Description == undefined ||
			this.Leave_Management_.Description == null ||
			this.Leave_Management_.Description == ""
		) {
			const dialogRef = this.dialogBox.open(DialogBox_Component, {
				panelClass: "Dialogbox-Class",
				data: { Message: "Enter Description", Type: "3" },
			});
			return;
		}

		if (
			this.Leave_Management_.FromDate == undefined ||
			this.Leave_Management_.FromDate == null 
		) {
			const dialogRef = this.dialogBox.open(DialogBox_Component, {
				panelClass: "Dialogbox-Class",
				data: { Message: "Choose From Date", Type: "3" },
			});
			return;
		}
		...
		if (
			this.Leave_Management_.ToDate == undefined ||
			this.Leave_Management_.ToDate == null 
		) {
			const dialogRef = this.dialogBox.open(DialogBox_Component, {
				panelClass: "Dialogbox-Class",
				data: { Message: "Choose To Date", Type: "3" },
			});
			return;
		}
		...
		if (
			this.Leave_Status_.Leave_Status_Id == 1 &&
			...
			const dialogRef = this.dialogBox.open(DialogBox_Component, {
				panelClass: "Dialogbox-Class",
				data: { Message: "Select Destination Staff", Type: "3" },
			});
			return;
		}
		// this.Cas_Followup_.Student_Id = this.cas_task_student_id;
		this.Leave_Management_.Department_Id = this.FollowUp_Department_Task_.Department_Id;
		this.Leave_Management_.Department_Name = this.FollowUp_Department_Task_.Department_Name;
		this.Leave_Management_.Leave_Status_Id = this.Leave_Status_.Leave_Status_Id;
		this.Leave_Management_.Status_Name = this.Leave_Status_.Status_Name;
		this.Leave_Management_.Source_User = this.Followup_Users_Task_.User_Details_Id;
		this.Leave_Management_.Source_User_Name =  this.Followup_Users_Task_.User_Details_Name;
		this.Leave_Management_.Leave_Management_Id = this.Leave_Management_Id;
		this.Leave_Management_.Branch_Id = this.FollowUp_Branch_Task_.Branch_Id;
			this.Leave_Management_.Branch_Name = this.FollowUp_Branch_Task_.Branch_Name;
			this.Leave_Management_.By_User_Id = Number(this.Login_User);
		this.Leave_Management_.By_User_Name = this.Login_User_Name;
		// this.Cas_Followup_.Task_Item_Id = this.cas_task_item_id;
		// this.Cas_Followup_.Task_Group_Id = this.cas_task_group_id;
		this.Leave_Management_.FromDate = this.New_Date(
			new Date(moment(this.Leave_Management_.FromDate).format("YYYY-MM-DD"))
		);
		this.Leave_Management_.ToDate = this.New_Date(
			new Date(moment(this.Leave_Management_.ToDate).format("YYYY-MM-DD"))
		);
		if (this.Leave_Status_.Leave_Status_Id == 1) {
			this.Leave_Management_.Destination_User_Id = this.Followup_Users_.User_Details_Id;
			this.Leave_Management_.Destination_User_Name = this.Followup_Users_.User_Details_Name;
		}
		debugger
		if (this.Save_Call_Status == true) return;
		else this.Save_Call_Status = true;
		this.issLoading = true;
		this.Student_Service_.Save_Leave_Management(this.Leave_Management_).subscribe(
			(Save_work_experience) => {
				Save_work_experience = Save_work_experience[0];
				if (Number(Save_work_experience[0].Leave_Management_Id_) > 0) {
					this.Save_Call_Status = false;
					const dialogRef = this.dialogBox.open(DialogBox_Component, {
						panelClass: "Dialogbox-Class",
						data: { Message: "Saved", Type: "false" },
 ...
 if (Number(Save_work_experience[0].Task_Status_) == 2) {
	...
	showTransferOptions() {
		this.isTransferVisible = true; // Show the transfer options
	  }
...
Search_Department_User_Typeahead_Destination(event: any, source: number) {
    var Value = "";
    ...
    if (
        ...
        this.Student_Service_.Search_Department_User_Typeahead(
            this.FollowUp_Branch_Task_.Branch_Id, 
            this.FollowUp_Department_Task_.Department_Id, 
            ""
        ).subscribe(
            (Rows) => {
                if (Rows != null) {
        ...
        if (source === 1) {
            ...
            this.Followup_Users_Data_t_Filter = [...this.Followup_Users_Data_t]; // Copy existing data
        }
    }
}
...
display_Followup_Users1(Users_: User_Details) {
if (Users_) {
return Users_.User_Details_Name;
}
}
