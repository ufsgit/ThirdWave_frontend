...
@Component({
selector: 'app-Branchwise_Summary',
templateUrl: './Branchwise_Summary.component.html',
styleUrls: ['./Branchwise_Summary.component.css'],
providers: [
...
export class Branchwise_SummaryComponent implements OnInit {
...
myInnerHeight: number;
    ...
    Branch_title = '';
    Branch_type = 'PieChart';
    Branchwise_data = [
      
    ];
    Branchwise_columnNames = [];
    Branch_options = {
      is3D: true,
    };
    width = 550;
...
var value = 1, dept_id=0,User_Id=0,search_name_='0',look_In_Date_Value=0,branch_id=0;
    ...
    this.Student_Service_.Search_Branchwise_Summary(moment(this.Search_FromDate).format('YYYY-MM-DD'), moment(this.Search_ToDate).format('YYYY-MM-DD'), value, search_name_, dept_id,branch_id, User_Id,   look_In_Date_Value, this.Black_Start, this.Black_Stop, this.Login_User, this.Red_Start, this.Red_Stop)
.subscribe(Rows => 
{
    ...
    { 
        result.push([Branchwise_data_temp[i].Branch_Name, Branchwise_data_temp[i].Branch_Total]);
    ...
    this.Branchwise_columnNames.push('Branch')
    this.Branchwise_columnNames.push('Amount')
    this.Branchwise_data = result;  
    this.Data_Bar=result;     
  
    if (this.Branchwise_data.length ==0) {
