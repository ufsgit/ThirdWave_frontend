...
import { Report_Data } from '../../../models/Report_Data';
...
providers: [
 ...
 export class Work_HistoryComponent implements OnInit {
    ...
    Report_Data_:Report_Data=new Report_Data();
...
Page_Load()
{
   ...
   this.Get_Menu_Status(58,this.Login_User);
  ...
  // this.Search_Student_Report();
   ...
   this.Show_FollowUp=false
    ...
    this.Search_Work_History()

    

 this.myInnerHeight = (window.innerHeight);
 this.myTotalHeight=this.myInnerHeight - 150;
...
Export()
{
        this.Student_Service_.exportExcel(this.Student_Data_Search,'Work History')
...
Search_Work_History()
{
   ...
   var c=0;
    ...
    this.Student_Service_.Search_Work_History(moment(this.Search_FromDate).format('YYYY-MM-DD'),
...
.subscribe(Graph_Status => 
{
 ...
 //  log(Graph_Status)
    ...
    for(var j=0;j<Enquiry_Source_data_temp.length;j++){
        this.Total=Number(this.Total)+Number(Enquiry_Source_data_temp[j].Data_Count)
    }
    for(var j=0;j<Enquiry_Source_data_temp.length;j++){
        this.Total_Amount=Number(this.Total_Amount)+Number(Enquiry_Source_data_temp[j].Total)
    }
    for(var j=0;j<Enquiry_Source_data_temp.length;j++){
   ...
   // var data_temp = new google.visualization.DataTable(result);
    ...
    Work_History_Send_Mail(){
        
        this.Report_Data_.Report_Array=this.Student_Data_Search;
        this.Report_Data_.Login_Id=Number(this.Login_User);
        this.Report_Data_.Login_User=this.uname;
        this.issLoading = true;
        
    this.Student_Service_.Work_History_Send_Mail(this.Report_Data_).subscribe(Status => 
    {
