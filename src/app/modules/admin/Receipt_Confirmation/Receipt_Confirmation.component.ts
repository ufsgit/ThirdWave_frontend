...
import { Fees_Receipt_Status } from "app/models/Fees_Receipt_Status";
...
@Component({
	selector: "app-Receipt_Confirmation",
	templateUrl: "./Receipt_Confirmation.component.html",
	styleUrls: ["./Receipt_Confirmation.component.css"],
	providers: [
		{
			provide: DateAdapter,
...
export class Receipt_ConfirmationComponent implements OnInit {
	// private url = "http://localhost:3510";
	// private url = 'https://regdemoapi.adat.in/';
	//private url = 'https://regapi.adat.in/';
	...
	Page_Load() {
		...
		this.Get_Menu_Status(91, this.Login_User);
	...
	Search_Lead_button() {
		...
		this.Red_Stop = this.Page_Length_;
		// this.Search_Student_Report();
		this.Search_Receipt_Confirmation();
	}
	Search_More_Options() {
	...
	Search_Receipt_Confirmation() {
		...
		var User_Id = 0,
			...
			branch_id = 0,value=0,search_name_='0',
...
debugger
		this.Student_Service_.Search_Receipt_Confirmation(
			...
			(Graph_Status) => {
				...
				//this.applicationdetails_Id=Serach_Data[0].Application_details_Id;
				//this.Receiptamount=Serach_Data[0].Amount;
				this.Total_Entries = Serach_Data.length;
				...
				for (var i in Enquiry_Source_data_temp) {
					result.push([
						Enquiry_Source_data_temp[i].Enquiry_Source_Name,
						Enquiry_Source_data_temp[i].Data_Count,
					]);
				}
	...
	Get_Lead_Load_Data_ByUser(Login_User) {
		...
		this.Student_Service_.Get_Lead_Load_Data_ByUser(Login_User).subscribe(
			(Rows) => {
				...
				this.Search_Fees_Receipt_Status = this.Fees_Receipt_Status_Data[6];
	...
	Receipt_Approve(
		...
		dialogRef.afterClosed().subscribe((result) => {
			if (result == "Yes") {
				...
				if (applicationdetails_Id ==null)
				{
					applicationdetails_Id=0
				}
				
				
				this.Student_Service_.Receipt_Approve(
					Fees_Receipt_Id,
					this.Login_User,
					applicationdetails_Id,
					Receiptamount
					...
					(Save_status) => {
						...
						if (Number(Save_status[0][0].Fees_Receipt_Id_) > 0) {
							...
							this.Search_Receipt_Confirmation();
							if (Number(this.Login_User) != Save_status[0][0].To_User_Id_) {
								var message = {
									Student_Name: Save_status[0][0].Student_Name_,
									From_User_Name: Save_status[0][0].User_Name_,
									Notification_Type_Name:
										Save_status[0][0].Notification_Type_Name_,
									Entry_Type: Save_status[0][0].Entry_Type_,
									To_User: Save_status[0][0].To_User_Id_,
	...
	Remove_Receipt_Approval(Fees_Receipt_Id, index) {
		//    application_details_id_
		const dialogRef = this.dialogBox.open(DialogBox_Component, {
			panelClass: "Dialogbox-Class",
			data: {
				Message: "Do you want to Remove Approval ?",
				Type: true,
				Heading: "Confirm",
			},
		});
		...
		dialogRef.afterClosed().subscribe((result) => {
			if (result == "Yes") {
				this.issLoading = true;
				this.Student_Service_.Remove_Receipt_Approval(
					Fees_Receipt_Id
				).subscribe(
					(update_status) => {
