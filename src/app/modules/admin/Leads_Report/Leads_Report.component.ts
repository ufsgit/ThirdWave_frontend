...
@Component({
selector: 'app-Leads_Report',
templateUrl: './Leads_Report.component.html',
styleUrls: ['./Leads_Report.component.css'],
providers: [
...
export class Leads_ReportComponent implements OnInit {
...
Search_Lead_button2() 
{
    ...
    this.Red_Stop = this.Page_Length_;
    this.To_User_Id =0;
    this.Pending_FollowUp(this.User_Search.User_Details_Id,this.To_User_Id)
}
Search_More_Options()
{
...
Details_Button(Status_Id,To_User_Id){
    
    for (var i = 0; i < this.Status_Data.length; i++) {
        if (Status_Id== this.Status_Data[i].Department_Status_Id)
        this.Search_Status=this.Status_Data[i];
    }
    
    
    this.Pending_FollowUp(Status_Id,To_User_Id);
 }
Pending_FollowUp(Department_Status_Id,To_User_Id)
{
    debugger
...
var value = 1, dept_id=0,search_name_='0',look_In_Date_Value=0,branch_id=0 ,User_Id=0;
  ...
  if (this.Search_Status != undefined && this.Search_Status != null)
    ...
    this.issLoading = true;
    debugger
    this.Student_Service_.Lead_Summary_Report( dept_id,branch_id,User_Id, Department_Status_Id,this.Login_User,To_User_Id)
.subscribe(Rows => 
{
    debugger
    //log(Rows)
...
FollowUp_Summary()
{
    ...
    this.issLoading = true;
    debugger
    this.Student_Service_.Lead_Summary( Department_Status_Id,User_Id,this.Login_User)
.subscribe(Rows => 
{
    debugger
    //log(Rows)
