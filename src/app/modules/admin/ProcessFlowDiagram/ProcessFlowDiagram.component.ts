...
@Component({
  selector: "app-ProcessFlowDiagram",
  templateUrl: "./ProcessFlowDiagram.component.html",
  styleUrls: ["./ProcessFlowDiagram.component.css"],
})
export class ProcessFlowDiagramComponent implements OnInit {
  ...
  Save_Process() {
    ...
    for (let i in status) {
      if (status[i].checked) {
        departmentStatus.push({departmentStatusId: i});
...
debugger
  ...
  showStatusByProcessId(processId,Process_Name) {
    ...
    debugger
    // this.router.navigate(['/Process_Status_chart'], { queryParams: { Process_Name: Process_Name,processId: processId  }});
    this.router.navigate(['/Process_Status_chart'], { queryParams: { Process_Name: Process_Name, processId: processId }});

    // this.router.navigateByUrl('/Process_Status_chart');
