...
@Component({
selector: 'app-Department_Status_Report',
templateUrl: './Department_Status_Report.component.html',
styleUrls: ['./Department_Status_Report.component.css'],
providers:[
...
export class Department_Status_ReportComponent implements OnInit {
    ...
    Department_Data_Search: Student[]
...
Page_Load()
{
    ...
    this.Get_Menu_Status(129, this.Login_User);
    ...
    Search_Lead_button2() 
{
    ...
    this.Total_Rows=0;
    this.Red_Stop = this.Page_Length_;
    this.Search_Main_report(this.User_Search.User_Details_Id);
...
Export()
{
        this.Student_Service_.exportExcel(this.Student_Data_Search,'Department Status Report')

}
Export_Summary()
{
        this.Student_Service_.exportExcel(this.Department_Data_Search,'Department Status Summary')
...
debugger
    ...
    //     this.Department_Summary_Search.Department_Id == null ||
    ...
    this.Student_Service_.Search_DepartmentStatus_Summary(moment(this.Search_FromDate).format('YYYY-MM-DD'), moment(this.Search_ToDate).format('YYYY-MM-DD'), User_Id,this.Login_User, look_In_Date_Value,dept_id)
.subscribe(Rows => 
{
    
   // log(Rows)
    this.Department_Data_Search = Rows.returnvalue.Leads;
    this.Total_Entries=0;
    ...
    {
        result.push([Branchwise_data_temp[i].To_Staff, Branchwise_data_temp[i].Count]);
    } 
...
this.issLoading = false;
if(this.Department_Data_Search.length==0)
{
...
View_Details_Click(User_Details_Id,Department_Id_e,Status_Id_e){
    //this.issLoading =true;
    debugger
    ...
    //     if (Department_Id_e== this.Department_Data[i].Department_Id)
    //     this.Department_Summary_Search=this.Department_Data[i];
    // }
    for (var i = 0; i < this.Status_Data.length; i++) {
        if (Status_Id_e== this.Status_Data[i].Department_Status_Id)
      

        this.Status_Search=this.Status_Data[i];

        console.log('Status_Id_e: ', Status_Id_e);

    }
    this.Search_Main_report(User_Details_Id);
}

Search_Main_report(User_Id)
{debugger
    
    this.WorkSummary_Div=false
    this.Search_Div=true
    this.Graph=false
var value = 1, dept_id=0,search_name_='0',look_In_Date_Value=0,branch_id=0,status_id=0;
    if(this.Search_By_!=undefined && this.Search_By_!=null)
    if (this.Search_By_ != undefined && this.Search_By_ != null && this.Search_By_ != '')
    value=this.Search_By_;
    ...
    for (var i = 0; i < this.Department_Data.length; i++) {
        if (this.Department_Summary_Search.Department_Id == this.Department_Data[i].Department_Id)
...
console.log('status_id: ', status_id);
    this.issLoading = true;
    debugger
    //this.Student_Service_.Search_Main_report(moment(this.Search_FromDate).format('YYYY-MM-DD'), moment(this.Search_ToDate).format('YYYY-MM-DD'),value, search_name_, dept_id,branch_id, User_Id,   look_In_Date_Value, this.Black_Start, this.Black_Stop, this.Login_User, this.Red_Start, this.Red_Stop)
    this.Student_Service_.Search_DepartmentStatus_report(moment(this.Search_FromDate).format('YYYY-MM-DD'), moment(this.Search_ToDate).format('YYYY-MM-DD'),
     value, search_name_, dept_id,branch_id, User_Id,look_In_Date_Value, this.Black_Start, this.Black_Stop, this.Login_User, this.Red_Start, this.Red_Stop,status_id)
.subscribe(Rows => 
{debugger
