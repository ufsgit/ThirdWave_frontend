...
@Component({
  selector: "app-Process_Status_chart",
  templateUrl: "./Process_Status_chart.component.html",
  styleUrls: ["./Process_Status_chart.component.css"],
})
export class Process_Status_chartComponent implements OnInit {
  ...
  loadProcessStatus(processId) {
    this.issLoading = true;
    this.processService.getProcessStatusChartListByProcessId(processId).subscribe(
      (response: any) => {
        this.processStatusList = response.data.map((item: any) => ({
          ...item,
          isExpanded: false,
          subStatusList: []
        }));
        this.processStatusList.forEach((status: any) => {
          this.toggleExpansion(status); // Initialize the expansion state
        })
  ...
  // toggleExpansion(status: any) {
  //   debugger
  //   status.isExpanded = !status.isExpanded;
  //   console.log('status.isExpanded: ', status.isExpanded);
  //   if (status.isExpanded && !status.subStatusList.length) { 
  //     this.loadSubStatus(status);
  //   }
  // }

  toggleExpansion(status: any) {
    debugger; 
    // Ensure subStatusList is initialized
    status.subStatusList = status.subStatusList || [];
  
    // Toggle the expansion
    status.isExpanded = !status.isExpanded;
    console.log('status.isExpanded: ', status.isExpanded);
  
    // Check if expanded and subStatusList is empty 
    if (status.isExpanded && !status.subStatusList.length) {
      console.log('Loading subStatus for:', status);
      this.loadSubStatus(status);
    }
  }
  loadSubStatus(status: any) {
    console.log('status: ', status);
    debugger
    if (!status.subStatusList.length) {
      this.issLoading = true;
      // Replace with your actual API call
       // assign editing data to variable
    this.editingDepartmentStatus = status;
    ...
    debugger
      this.processService.Search_Process_NextStatus_Details_contain(this.editingDepartmentStatus.Department_Status_Id,this.editingDepartmentStatus.processId).subscribe(
        
        (response: any) => {
          debugger
          console.log('response: ', response);
          console.log(' status.subStatusList: ',  status.subStatusList);
          status.subStatusList = response[1];
          console.log('response[0][1]: ', response[1]);
       
          this.issLoading = false;
        },
        (error) => {
          console.error('Error loading sub status:', error);
          this.issLoading = false;
        }
      );
    }
  ...
  this.activatedRoute.queryParamMap.subscribe((params) => {
    ...
    const processId = params.get("processId");
    this.getDepartmentStatusDetailsProcessId(processId);
    this.loadProcessStatus(processId);
  });
...
//   break;
  ...
  goBackToDepartmentDetails() {
    this.router.navigateByUrl("ProcessFlowDiagram");
  }
